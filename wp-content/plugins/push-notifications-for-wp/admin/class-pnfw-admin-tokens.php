...
class Tokens_Table extends WP_List_Table {
 ...
 function column_token($item) {
  $actions = array(
      'delete' => sprintf('<a href="?page=%s&action=%s&id=%s&_wpnonce=%s">%s</a>', $_REQUEST['page'], 'delete', $item['id'], wp_create_nonce('delete' . $item['id']), __('Delete', 'push-notifications-for-wp'))
     );

     if (!pnfw_starts_with($item['token'], 'tokenless_')) {
      $actions['send_test_notification'] = sprintf('<a href="?page=%s&action=%s&id=%s&_wpnonce=%s">%s</a>', $_REQUEST['page'], 'send_test_notification', $item['id'], wp_create_nonce('send_test_notification' . $item['id']), __('Send test notification', 'push-notifications-for-wp'));
  }
