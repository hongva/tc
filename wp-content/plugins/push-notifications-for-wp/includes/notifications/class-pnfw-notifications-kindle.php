...
class PNFW_Notifications_Kindle extends PNFW_Notifications {
 ...
 protected function raw_send($tokens, $title, $user_info) {
  ...
  if (is_null($access_token)) {
   pnfw_log(PNFW_KINDLE_LOG, __("Can't obtain ADM access token. ADM Client ID or ADM Client Secret are probably incorrect.", 'push-notifications-for-wp'));
      return 0;
  }
 ...
 private function get_auth_token() {
  ...
  if (empty($clientId)) {
   pnfw_log(PNFW_KINDLE_LOG, __("ADM Client ID is not correctly set.", 'push-notifications-for-wp'));
      return null;
  }
  ...
  if (empty($clientSecret)) {
   pnfw_log(PNFW_KINDLE_LOG, __("ADM Client Secret is not correctly set.", 'push-notifications-for-wp'));
      return null;
  }
