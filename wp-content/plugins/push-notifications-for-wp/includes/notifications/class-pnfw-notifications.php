 ...
 public function send_post_to_user_categories($post) {
  ...
  $page = 0;
   ...
   if (empty($user_cat)) {
    ...
    $total += $this->raw_send($tokens, $title, $post->ID);
    ...
    $total += $this->raw_send($out_tokens, $title, $post->ID);
   }

   if (count($tokens) >= 1000) {
    pnfw_log($this->type, sprintf(__('This plugin allows you to send push notifications to up to %d tokens per platform. To send more notifications, please upgrade to <a href="http://www.delitestudio.com/wordpress/push-notifications-for-wordpress/">Push Notifications for Wordpress</a>.', 'push-notifications-for-wp'), 1000));
   }
